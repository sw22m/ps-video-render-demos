...
class ARtspMediaFactory(GstRtspServer.RTSPMediaFactory):
    ...
    def do_create_element(self, url):
        ...
        udp_src.set_property("port", 8004)
...
class VideoWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        udp_sink.set_property("port", RTSP_SERVER_PORT)
        ...
        server.props.service = f"{RTSP_SERVER_PORT}"
        server.attach(None)
        mount_point = "feed"

        # media_factory = ARtspMediaFactory.custom_pipeline_factory(self.player)
        media_factory = ARtspMediaFactory(rtph264_filter)
        media_factory.set_shared(True)
        # rtsp_url = GstRtsp.RTSPUrl()
        # rtsp_url.set_port(8554)
        self.player.set_state(Gst.State.PLAYING)
        server.get_mount_points().add_factory(f"/{mount_point}", media_factory)
        server.attach(None)

        url = f"rtsp://127.0.0.1:{RTSP_SERVER_PORT}/{mount_point}"
